import { ensureVanSchema } from '../../../src/runs/service';
...
export const GET=handler(async req=>{
 requireAdmin(req);const c=config();await ensureVanSchema();
 ...
 const runs=(await query(`SELECT local_date,state,event_id,error_code,updated_at,van_key FROM van_daily_runs WHERE account_id=$1 UNION ALL SELECT local_date,state,event_id,error_code,updated_at,'daily' AS van_key FROM daily_runs WHERE account_id=$1 ORDER BY local_date DESC LIMIT 20`,[c.JOBBER_ACCOUNT_ID])).rows;
