...
type Status={today:string;connection:string;writesEnabled:boolean;runs:{local_date:string;state:string;event_id:string|null;error_code:string|null}[]};
export default function Home() {
 ...
 return <main>
  ...
  <section className="intro"><p className="eyebrow">READY FOR THE WORKDAY</p><h2>The right materials.<br/>A clearer morning.</h2><p>Daily Jobber scope, organized by employee, with a shared buying list and clear questions to verify.</p></section>
  {!status?<section className="card login"><h3>Team access</h3><p>Sign in with the internal access secret configured for this app.</p><form onSubmit={e=>{e.preventDefault();action(async()=>{await request('/api/auth/login',{secret});setSecret('');await refresh();});}}><label htmlFor="secret">Access secret</label><input id="secret" type="password" autoComplete="current-password" value={secret} onChange={e=>setSecret(e.target.value)} required/><button disabled={busy}>Sign in</button></form><p className="muted">First setup? Follow SETUP.md in the project to configure the database and environment.</p></section>:<>
   <section className="stats"><article><small>JOBBER CONNECTION</small><strong>{status.connection.replaceAll('_',' ')}</strong></article><article><small>MORNING PREPARATION</small><strong>5:45 AM · Mon–Fri</strong></article><article><small>CALENDAR EVENT</small><strong>6:00 AM · Pacific</strong></article></section>
   <section className="card"><div className="sectionhead"><div><h3>Prepare a material list</h3><p>Preview reads Jobber and generates a list. Create writes the daily calendar event.</p></div><span className={status.writesEnabled?'badge live':'badge'}>{status.writesEnabled?'Writes enabled':'Preview mode'}</span></div>
    ...
    <div className="actions"><button disabled={busy||!date} onClick={()=>action(async()=>{const data=await request('/api/manual-test',{date,dryRun:true});setOutput(data.description??JSON.stringify(data,null,2));})}>Preview materials</button><button className="secondary" disabled={busy||!date||!status.writesEnabled} onClick={()=>action(async()=>{const data=await request('/api/manual-test',{date,dryRun:false});setOutput(JSON.stringify(data,null,2));await refresh();})}>Create daily event</button><button className="link" disabled={busy} onClick={()=>action(async()=>{const data=await request('/api/oauth/start',{});window.location.assign(data.url);})}>Connect / reconnect Jobber</button></div>
   ...
   <section className="card"><div className="sectionhead"><h3>Recent runs</h3><button className="link" disabled={busy} onClick={()=>action(refresh)}>Refresh</button></div><div className="tablewrap"><table><thead><tr><th>Date</th><th>Status</th><th>Action needed</th></tr></thead><tbody>{status.runs.length?status.runs.map(r=><tr key={r.local_date}><td>{r.local_date}</td><td>{r.state}</td><td>{['creating','uncertain'].includes(r.state)?'Check Jobber and follow the reconciliation guide.':r.error_code??'—'}</td></tr>):<tr><td colSpan={3}>No calendar runs yet. Start with a preview.</td></tr>}</tbody></table></div></section>
   <button className="link" disabled={busy} onClick={()=>action(async()=>{await request('/api/auth/logout',{});setStatus(null);setOutput('');})}>Sign out</button>
  </>}
